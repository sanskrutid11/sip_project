...
def calculate_investment():
    try:
        ...
        result_label.config(text=f"Future Price of the Car: ₹{future_price:,.2f}")
        investment_label.config(text=f"To achieve your dream car goal of ₹{future_price:,.2f}:")
        sip_label.config(text=f"Monthly SIP Required: ₹{monthly_sip:,.2f}")

        result_section.pack(fill="x", pady=20)  # Show result section dynamically
...
header.pack(fill="x")
header_label = tk.Label(header, text="Dream Car Planner", font=("Arial", 20, "bold"), fg=HEADER_TEXT_COLOR, bg=HEADER_BG_COLOR)
...
# Year
year_label = tk.Label(input_frame, text="I wish to buy my dream car in the year:", font=("Arial", 14), bg=CONTAINER_BG_COLOR, fg=LABEL_TEXT_COLOR)
...
# Price
price_label = tk.Label(input_frame, text="Estimated price of the car (as per today):", font=("Arial", 14), bg=CONTAINER_BG_COLOR, fg=LABEL_TEXT_COLOR)
...
# Risk Profile
risk_label = tk.Label(input_frame, text="My risk profile is:", font=("Arial", 14), bg=CONTAINER_BG_COLOR, fg=LABEL_TEXT_COLOR)
risk_label.grid(row=3, column=0, padx=10, pady=10, sticky="w")
...
risk_var.set("Medium")
risk_dropdown.grid(row=3, column=1, padx=10, pady=10)
...
footer.pack(fill="x")
footer_label = tk.Label(footer, text="\u00A9 2025 Dream Car Planner. All Rights Reserved.", font=("Arial", 12), bg=FOOTER_BG_COLOR, fg=FOOTER_TEXT_COLOR)
